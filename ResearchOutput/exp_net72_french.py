#!/usr/bin/env python3
# NET-72 — THE FRENCH DOMAIN TEST (limited-memory axis, iteration 40)
# Fourth domain-jump leg: French prose (romance language, different branch than German's
# germanic). Tests whether the tokenizer-tax direction is language-general or
# germanic-specific, and whether the +4 shift replicates.
#
# PREDICTIONS STATED BEFORE THE RUN:
#  P1 ROMANCE-TAX-MATCHES-GERMANIC: French knees = {20, 24} exactly (the +4 shift is
#     language-general for non-English prose).
#  P2 ROMANCE-LIGHTER: French knees = {16, 20} (romance languages tokenize more efficiently
#     than germanic compounds, matching English prose).
#  P3 BETWEEN: knees at {16 or 20, 20 or 24} mixed.
...
def fetch_french():
    if os.path.exists(FR_CACHE) and os.path.getsize(FR_CACHE) > 1_000_000:
        return open(FR_CACHE, encoding="utf-8", errors="ignore").read()
    urls = ["https://www.gutenberg.org/files/1608/1608-0.txt",    # Les Misérables vol 1? no — Romeo? check
            "https://www.gutenberg.org/files/17489/17489-0.txt"]  # Dumas or Hugo fallback
    parts = []
    for u in urls:
        try:
            ...
            parts.append(t)
            print(f"[fr] {u.split('/')[-1]}: {len(t)} chars", flush=True)
        except Exception as e:
            print(f"[fr] FAILED {u}: {e}", flush=True)
    s = "\n\n".join(parts)
    assert len(s) > 300_000, f"french corpus too small: {len(s)}"
    with open(FR_CACHE, "w", encoding="utf-8") as fh:
...
def main():
    ...
    for ctx in CTXS:
        ...
        for k in GRIDS[ctx]:
            ...
            json.dump(res, open(os.path.expanduser("~/f3cache/net72_results.json"), "w"), indent=1)
            torch.cuda.empty_cache()
