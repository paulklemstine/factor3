#!/usr/bin/env python3
# NET-50 — THE TROPICAL LIMIT OF REAL ATTENTION (limited-memory axis, iteration 2;
# mined from the Lean catalogue: Maslov sandwich, softmax->argmax, crystallization budgets)
#
# Question: NET-49 measured knees {16,32,24} at ctx {512,1024,2048} on Qwen2.5-0.5B —
# but stopped above k=16. The catalogue's tropical results say softmax attention is
# within log 2 nats of its argmax per row (Maslov sandwich) and that crystallization
# loss sum p(1-p) controls soft-vs-hard attention TV distance. This round measures THE
# TROPICAL LIMIT ITSELF: sweep oracle top-k down to k=1,2,4,8 and directly measure the
# per-row Maslov gap (LSE - max of masked scores) and crystallization loss per layer.
#
# Protocol: same harness/gates as NET-49 (forward validated vs HF before measurement,
# held-out last 10%, 40 disjoint windows, retained >= 0.98 bar). Sweep k in
# {1,2,4,8,16,32,...}; Part B tropical budgets from full-attention rows per layer.
#
...
MODEL_ID = "/tmp/qwen25-05b" if os.path.exists("/tmp/qwen25-05b/model.safetensors") \
    else "Qwen/Qwen2.5-0.5B"
PREDICTIONS = """
PRE-REGISTERED PREDICTIONS (stated before any measurement):
 P1 TROPICAL-CLIFF: pure argmax attention (k=1) is catastrophic on a real LM ->
    retained accuracy < 0.5 at EVERY context (the tropical limit is lossy per row).
 P2 SMALL-K-RECOVERY: a handful of keys repairs most of the damage ->
    k=4 already retains >= 0.90 at ctx=512, and k=8 >= 0.90 at ctx=2048.
 P3 NEAR-TROPICAL-SOFTMAX: the median per-row Maslov gap (LSE - max) is <= log 8 ~ 2.08
    nats at every context, and the median crystallization loss sum p(1-p) <= 0.25 ->
    softmax rows are already close to their own argmax; the small-k cliff of P2 is the
    aggregate shadow of a few heavy-gap layers/rows, not a uniform property.
"""
...
def fetch_corpus(cache="/tmp/net49_corpus.txt"):
        ...
        for li, layer in enumerate(self.layers):
            ...
            for qs in range(0, L, QC):
                ...
                if collect is not None:
                    ...
                    # NET-50 tropical budgets, per query row:
                    scf = sc.float().masked_fill(~causal[qs:qe], float("-inf"))
                    gap = torch.logsumexp(scf, dim=-1) - scf.max(dim=-1).values   # LSE - max (Maslov)
                    p2 = 1.0 - (pf * pf).sum(-1)                              # crystallization loss sum p(1-p)
                    w = qe - qs
                    c = collect.setdefault(li, {"ent": 0.0, "t64": 0.0, "t256": 0.0,
                                                "gap": 0.0, "gmed": 0.0, "xtal": 0.0,
                                                "g90": 0.0, "w": 0})
                    c["ent"] += float(ent.mean()) * w
                    c["t64"] += float(srt[..., :64].sum(-1).mean()) * w
                    c["t256"] += float(srt[..., :256].sum(-1).mean()) * w
                    c["gap"] += float(gap.mean()) * w
                    c["gmed"] += float(gap.median()) * w
                    c["g90"] += float(gap.quantile(0.9)) * w                 # 90th pctile Maslov gap
                    c["xtal"] += float(p2.mean()) * w
                    c["w"] += w
                    del pf, srt, ent, scf, gap, p2
...
def main():
    ...
    grids = {512: [1, 2, 4, 8, 16, 32, 64],
             1024: [1, 2, 4, 8, 16, 32, 64, 128],
             2048: [1, 2, 4, 8, 16, 24, 32, 64, 128, 256]}
    randks = {}
    locals_ = {}

    res_path = "/tmp/net50_results.json"
    ...
    for ctx in ctxs:
        ...
        # Part A: per-layer concentration + NET-50 tropical budgets
        ent_l, t64_l, t256_l, eff = [], [], [], []
        gap_l, gmed_l, g90_l, xtal_l = [], [], [], []
        for li in sorted(col.keys()):
            ...
            gap_l.append(round(c["gap"] / w, 4)); gmed_l.append(round(c["gmed"] / w, 4))
            g90_l.append(round(c["g90"] / w, 4)); xtal_l.append(round(c["xtal"] / w, 4))
        results[f"stats_ctx{ctx}"] = {"eff_support_per_layer": eff,
                                      "top64_mass": t64_l, "top256_mass": t256_l,
                                      "maslov_gap_mean_per_layer": gap_l,
                                      "maslov_gap_median_per_layer": gmed_l,
                                      "maslov_gap_p90_per_layer": g90_l,
                                      "crystallization_loss_per_layer": xtal_l}
        print(f"[full ] acc={full_acc:.4f} ce={full_ce:.4f}", flush=True)
        print(f"[partA] eff/layer L0..L{len(eff)-1}: {eff}", flush=True)
        print(f"[maslov] gap mean/layer: {gap_l}", flush=True)
        print(f"[maslov] gap median/layer: {gmed_l}", flush=True)
        print(f"[maslov] gap p90/layer: {g90_l}", flush=True)
        print(f"[xtal] crystallization loss sum p(1-p) /layer: {xtal_l}", flush=True)
