#!/usr/bin/env python3
"""UNIVERSAL-S3-CORRECTED — the type-channel law for x3-2, degree 3 (round-32 #3)."""
...
print("=== UNIVERSAL-S3-CORRECTED (round-32 #3): x3-2, degree 3 ===",flush=True)
pf=sieve(1<<16)
prp=pf[~np.isin(pf,[2,3])]  # ramified: 2, 3
NM=30000

# x3-2: coeffs const-first (-2, 0, 0, 1) = -2 + 0x + 0x2 + 1x3
coeffs=(-2,0,0,1)
...
# sign character at conductor 3: (-3|p) determines even/odd Frobenius
pm3=(prp%3).astype(np.int64)
I_sign=mi(pm3,types)
H_cond=H_T-I_sign
print(f"\n  I(p mod 3; T)={I_sign:.4f} (sign contribution)",flush=True)
print(f"  H(T|sign)={H_cond:.4f}",flush=True)
