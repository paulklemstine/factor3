...
# -*- coding: utf-8 -*-
"""
exp531_t_dial_unif_68.py -- TDIAL-U68 (round-62, experiment 531)

Push the zero-fit dial T(N) = sum_{odd QR primes p<=400} 2/p to BITLEN 68
on UNIFORM draws (prior uniform cells: 44 -> exp517, 48 -> exp527,
52 -> exp518/528, 56 -> exp529, 60 -> exp521, 64 -> exp530; balanced
cells through bitlen 60).
...
DESIGN (as assigned)
==========
Work dir    : /tmp/exp62_tu68/   (protocol: work ONLY here; never write to
                                 /home/raver1975/factor3)
Population  : 1200 uniform semiprimes per seed; seeds 20261150, 20261151,
              20261152; N rejected unless bit_length == 68 exactly.
...
=====================================================================
The brief specifies p ~ uniform prime in [2^18,2^24), q ~ uniform prime in
[2^24,2^30), AND N of bitlen exactly 68. These are incompatible: the largest
product of the stated windows is < 2^24 * 2^30 = 2^54, so NO draw can reach
bitlen 68 (bitlen-68 rejection sampling would never terminate). This is the
...
too low for its target bitlen).
Correction applied before data collection, following exp529/exp530's rule
...
distortion):
midpoints 21 + 27 = 48  ->  shift = (68 - 48)/2 = +10 ->
        p ~ uniform prime in [2^28, 2^34)
        q ~ uniform prime in [2^34, 2^40)
...
and places the product distribution's MODE exactly at 2^68 (window-log
midpoints 31 + 37 = 68). Everything else in the brief is implemented
verbatim. FLAGGED for parent adjudication.

PROTOCOL NOTE (flagged): this agent READ (never wrote) three artifacts of
the immediate predecessor cell -- the exp530 script (repo scripts dir),
...
Implementation deltas vs exp530, both forced by bitlen 68 and neither a fit:
 * sieve ceiling 20000 -> 30000: B_N = (2*sqrt(N))^(1/2.5) reaches just
   under 2^14 = 16384 at bitlen 68 (vs ~9.7e3 at 64); the factor base must
   cover B_N.
 * everything else byte-for-byte the exp530 pipeline (population draw order,
...
VERDICT NAMES (pre-stated):
  H1 pass & H2 pass -> U68-DIAL-CONFIRMED
  H1 only           -> U68-DIAL-HOLDS-COUNT-PARITY
  H2 only           -> U68-DIAL-SHIFT-COUNT-ROBUST
  neither           -> U68-DIAL-BREAK
...
SCRIPT_PATH = os.path.join(WD, "exp531_t_dial_unif_68.py")

# ------------------------------------------------- fixed configuration
SEEDS = [20261150, 20261151, 20261152]
TARGET_BITS = 68
P_WIN = (28, 34)          # corrected window, see DESIGN CORRECTION above
Q_WIN = (34, 40)          # corrected window
...
SIEVE_CEIL = 30000        # covers B_N max ~1.6e4 at bitlen 68 (see header)
...
CORRECTION_TEXT = (
    "Brief's windows p~[2^18,2^24), q~[2^24,2^30) admit max product < 2^54 -- "
    "bitlen-68 rejection sampling is infeasible as stated (same copy-carried "
    "defect documented pre-data in exps 517/518/521/528/529/530). Pre-data "
    "correction per exp529/exp530's rule: both exponent windows shifted by "
    "the SAME +10, chosen so window-log midpoints sum to 68 (21+27=48 -> "
    "31+37=68) -> p~[2^28,2^34), q~[2^34,2^40); preserves adjacency/"
    "equal-widths/p<q/uniform character; product-mode lands on 2^68. "
    "FLAGGED for parent."
)
...
# ------------------------------------------------------- state / io
STATE = {
    "experiment": "exp531", "codename": "TDIAL-U68", "round": 62,
...
def main():
    ...
    STATE["config"] = {
        ...
        "windows_as_written_in_brief": [[18, 24], [24, 30]],
    ...
    if h1v and h2v:
        name = "U68-DIAL-CONFIRMED"
    elif h1v and not h2v:
        name = "U68-DIAL-HOLDS-COUNT-PARITY"
    elif (not h1v) and h2v:
        name = "U68-DIAL-SHIFT-COUNT-ROBUST"
    else:
        name = "U68-DIAL-BREAK"
    ...
    STATE["barriers"] = {
        ...
        "barrier_8_zero_fit_discipline":
            ...
            "bitlen-68-uniform is an out-of-sample intersection fill; no "
...
def write_ledger_md(name, h1v, h2v):
    lines = [
        "# LEDGER -- exp531 TDIAL-U68 (round-62)",
        "",
        f"- date_utc: {now()}",
