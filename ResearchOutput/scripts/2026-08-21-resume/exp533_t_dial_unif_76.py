...
# -*- coding: utf-8 -*-
"""
exp533_t_dial_unif_76.py -- TDIAL-U76 (round-64, experiment 533)

Push the zero-fit dial T(N) = sum_{odd QR primes p<=400} 2/p to BITLEN 76
on UNIFORM draws (prior uniform cells: 44 -> exp517, 48 -> exp527,
52 -> exp518/528, 56 -> exp529, 60 -> exp521, 64 -> exp530,
68 -> exp531, 72 -> exp532; balanced cells through bitlen 60).
...
DESIGN (as assigned)
==========
Work dir    : /tmp/exp64_tu76/   (protocol: work ONLY here; never write to
                                 /home/raver1975/factor3)
Population  : 1200 uniform semiprimes per seed (uniform-cell convention of
              exps 517..532); seeds 20261170, 20261171, 20261172; N rejected
              unless bit_length == 76 exactly.
...
=====================================================================
The brief specifies p ~ uniform prime in [2^22,2^28), q ~ uniform prime in
[2^28,2^34), AND N of bitlen exactly 76. These are incompatible: the largest
product of the stated windows is < 2^28 * 2^34 = 2^62, so NO draw can reach
bitlen 76 (bitlen-76 rejection sampling would never terminate). This is the
same copy-carried template defect already documented pre-data in exps 517,
518, 521, 528, 529, 530, 531 and 532 (each round's brief carries windows two
octaves too low for its target bitlen).
Correction applied before data collection, following exp529..532's rule
...
distortion):
midpoints 25 + 31 = 56  ->  shift = (76 - 56)/2 = +10 ->
        p ~ uniform prime in [2^32, 2^38)
        q ~ uniform prime in [2^38, 2^44)
...
and places the product distribution's MODE exactly at 2^76 (window-log
midpoints 35 + 41 = 76). Everything else in the brief is implemented
verbatim. FLAGGED for parent adjudication.

PROTOCOL NOTE (flagged): this agent READ (never wrote) two artifacts of
the immediate predecessor cell -- the exp532 script (repo scripts dir,
read-only) and its /tmp/exp63_tu72 result.json -- to guarantee verbatim
protocol fidelity and to fill the prior-cells context. No repo file was
modified or created.

Implementation deltas vs exp532, both forced by bitlen 76 and neither a fit:
 * sieve ceiling 50000 -> 60000: B_N = (2*sqrt(N))^(1/2.5) reaches
   ~2^15.6 = 5.00e4 at bitlen 76 (vs ~2.89e4 at 72); the factor base must
   cover B_N.
 * everything else byte-for-byte the exp532 pipeline (population draw order,
...
VERDICT NAMES (pre-stated):
  H1 pass & H2 pass -> U76-DIAL-CONFIRMED
  H1 only           -> U76-DIAL-HOLDS-COUNT-PARITY
  H2 only           -> U76-DIAL-SHIFT-COUNT-ROBUST
  neither           -> U76-DIAL-BREAK
...
SCRIPT_PATH = os.path.join(WD, "exp533_t_dial_unif_76.py")

# ------------------------------------------------- fixed configuration
SEEDS = [20261170, 20261171, 20261172]
TARGET_BITS = 76
P_WIN = (32, 38)          # corrected window, see DESIGN CORRECTION above
Q_WIN = (38, 44)          # corrected window
...
SIEVE_CEIL = 60000        # covers B_N max ~5.0e4 at bitlen 76 (see header)
...
CORRECTION_TEXT = (
    "Brief's windows p~[2^22,2^28), q~[2^28,2^34) admit max product < 2^62 -- "
    "bitlen-76 rejection sampling is infeasible as stated (same copy-carried "
    "defect documented pre-data in exps 517/518/521/528/529/530/531/532). "
    "Pre-data correction per exp529..532's rule: both exponent windows shifted "
    "by the SAME +10, chosen so window-log midpoints sum to 76 (25+31=56 -> "
    "35+41=76) -> p~[2^32,2^38), q~[2^38,2^44); preserves adjacency/"
    "equal-widths/p<q/uniform character; product-mode lands on 2^76. "
    "FLAGGED for parent."
)
...
def smooth_count_sq(v, fb, B):
    ...
    for speed; identical semantics to the exp528..exp532 classifier.)"""
    x = v
    for pr, pr2 in fb:
        if pr2 > x:
            break
...
# ------------------------------------------------------- state / io
STATE = {
    "experiment": "exp533", "codename": "TDIAL-U76", "round": 64,
...
def main():
    ...
    STATE["config"] = {
        ...
        "windows_as_written_in_brief": [[22, 28], [28, 34]],
    ...
    if h1v and h2v:
        name = "U76-DIAL-CONFIRMED"
    elif h1v and not h2v:
        name = "U76-DIAL-HOLDS-COUNT-PARITY"
    elif (not h1v) and h2v:
        name = "U76-DIAL-SHIFT-COUNT-ROBUST"
    else:
        name = "U76-DIAL-BREAK"
    ...
    STATE["barriers"] = {
        ...
        "barrier_8_zero_fit_discipline":
            ...
            "bitlen-76-uniform is an out-of-sample intersection fill; no "
        ...
        "note": "wording reconstructed from lab semantics (barrier 5 = "
                ...
                "papers 83/183/184 usage and the exp529..exp532 "
...
def write_ledger_md(name, h1v, h2v):
    lines = [
        "# LEDGER -- exp533 TDIAL-U76 (round-64)",
        "",
        f"- date_utc: {now()}",
