#!/usr/bin/env python3
"""DEGREE-6-NONABELIAN — D6 type channel for x6-2 (round-34 #4)."""
...
print("=== DEGREE-6-NONABELIAN (round-34 #4): x6-2, D6 ===",flush=True)
pf=sieve(1<<16)
prp=pf[~np.isin(pf,[2,3])]  # ramified: 2, 3
NM=30000

coeffs=(-2,0,0,0,0,0,1)
...
for m in (3,5,7,13):
    ...
    v=mi(pm,types)
    rng_np=np.random.default_rng(m*100+3)
    nl=[]
...
print(f"  I(N mod 3; pair)={I_pair:.4f} | wall z={z_pair:+.2f}",flush=True)
...
print("Round-34 #4.",flush=True)
print("\nALL_DONE_R34N4",flush=True)
