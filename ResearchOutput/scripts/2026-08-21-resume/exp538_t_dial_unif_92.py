...
# -*- coding: utf-8 -*-
"""
exp538_t_dial_unif_92.py -- TDIAL-U92 (round-69, experiment 538)

THE NEXT LADDER RUNG after exp536 (round-68): push the zero-fit dial
T(N) = sum_{odd QR primes p<=400} 2/p to BITLEN 92 on UNIFORM draws.
...
84 -> exp535 (paper 188: pooled rho_T = 0.558, margin to floor +0.008),
88 -> exp536 (pooled rho_T = 0.534, CI [0.509, 0.555] -- BELOW the 0.55
floor for the first time; DRIFT-INCONCLUSIVE). This cell decides whether
that drop is TERMINAL DRIFT or a PLATEAU.

=====================================================================
PRE-STATED HYPOTHESES (written BEFORE any data collection; verbatim
from the round-69 brief)
=====================================================================
H1: Spearman(T, rate) drops decisively below 0.50 at bitlen 92 --
    the drift is terminal.
H2: Spearman(T, rate) stays >= 0.50 at bitlen 92 -- the drift plateaus
    near the floor.

Operationalization (fixed before data):
  H1 PASS iff pooled rho_T < 0.50 AND pooled bootstrap CI upper < 0.50
     ("decisively below": point AND interval entirely under 0.50).
  H2 PASS iff pooled rho_T >= 0.50 (point at or above the H1/H2 line).
Verdict names (pre-stated, mutually exclusive, exhaustive):
  rho < 0.50, CI upper < 0.50   -> U92-TERMINAL-DRIFT      (H1 pass)
  rho < 0.50, CI straddles 0.50 -> U92-DRIFT-INCONCLUSIVE  (neither)
  0.50 <= rho < 0.55            -> U92-PLATEAU-AT-FLOOR    (H2 pass)
  rho >= 0.55                   -> U92-PLATEAU-ABOVE-FLOOR (H2 pass;
                                    recovery above the ladder floor)
Secondary (reported, not gating): per-seed corroboration counts
(#/3 seeds with rho_T >= 0.50 and #/3 with rho_T >= 0.55).

==========
DESIGN (as assigned)
==========
Work dir    : /tmp/exp69_tu92/  (protocol: work ONLY here; never write
                                 to /home/raver1975/factor3)
Population  : 1200 uniform semiprimes per seed; seeds 20261210,
              20261211, 20261212; N rejected unless bit_length == 92
              exactly (rejection sampling, restart BOTH draws on reject --
              the unbiased conditional population of exps 517..536).
...
=====================================================================
The brief specifies p ~ uniform prime in [2^36,2^42), q ~ uniform prime
in [2^42,2^48), AND N of bitlen exactly 92. These are incompatible: the
largest product of the stated windows is < 2^42 * 2^48 = 2^90, so NO
draw can reach bitlen 92 (bitlen-92 rejection sampling would never
...
533, 534, 535 and 536 (each round's brief carries windows too low for
its target bitlen; the stated windows here center bitlen 84 -- log
midpoints 39 + 45).
Correction applied before data collection, following exp529..536's rule
...
distortion):
midpoints 39 + 45 = 84  ->  shift = (92 - 84)/2 = +4 ->
        p ~ uniform prime in [2^40, 2^46)
        q ~ uniform prime in [2^46, 2^52)
...
and makes window-log midpoints sum exactly to 92 (the correction rule),
continuing exp536's cell ([2^38,2^44) x [2^44,2^50)) by the same
+2-exponent step per +4-bit rung the ladder has used throughout.
Analytic acceptance computed BEFORE data: for linear-uniform draws the
bitlen-92 slice is P(u*v in [2^5, 2^6)), u,v ~ U(1,64) =
...
the immediate predecessor cell -- the exp536 script (repo ResearchOutput
tree, read-only) -- to guarantee verbatim protocol fidelity ("features
verbatim paper-164") and to fill the prior-cells context. No repo file
was modified or created.

Implementation deltas vs exp536, none a fit:
 * factor-base ceiling 350000 -> 600000: B_N = (2*sqrt(N))^(1/2.5)
   reaches ~2^18.8 = 456436 at bitlen 92 (vs 2^18 = 262144 at 88);
   the factor base must cover B_N (max B_N over the population
   456436 << 600k, same ~1.3x headroom rule as exp536).
 * classifier cross-check bounds rescaled to the bitlen-92 base sizes;
 * everything else byte-for-byte the exp536 pipeline (population draw
   ...
   bootstrap seed scheme, designed checks).
...
SCRIPT_PATH = os.path.join(WD, "exp538_t_dial_unif_92.py")

# ------------------------------------------------- fixed configuration
SEEDS = [20261210, 20261211, 20261212]
TARGET_BITS = 92
P_WIN = (40, 46)          # corrected window, see DESIGN CORRECTION above
Q_WIN = (46, 52)          # corrected window
...
FB_CEIL = 600000          # covers B_N max ~2^18.8 = 456436 at bitlen 92
...
LINE = 0.50               # H1/H2 boundary (this brief)
LADDER_FLOOR = 0.55       # prior ladder floor (context / secondary split)
ANCHOR_88_RHO = 0.534     # exp536 bitlen-88 pooled anchor
ANCHOR_88_CI = [0.509, 0.555]
...
HYPOTHESES = {
    "H1-terminal": "Spearman(T, rate) drops decisively below 0.50 at "
                   "bitlen 92 -- the drift is terminal.",
    "H2-plateau": "Spearman(T, rate) stays >= 0.50 at bitlen 92 -- the "
                  "drift plateaus near the floor.",
}

CORRECTION_TEXT = (
    "Brief's windows p~[2^36,2^42), q~[2^42,2^48) admit max product "
    "< 2^90 -- bitlen-92 rejection sampling is infeasible as stated "
    "(same copy-carried defect documented pre-data in exps 517/518/521/"
    "528/529/530/531/532/533/534/535/536; stated windows center bitlen "
    "84). Pre-data correction per exp529..536's rule: both exponent "
    "windows shifted by the SAME +4, chosen so window-log midpoints sum "
    "to 92 (39+45=84 -> 43+49=92) -> p~[2^40,2^46), q~[2^46,2^52); "
    "preserves adjacency/equal-widths/p<q/uniform character; analytic "
    "pre-data acceptance (224*ln2-32)/63^2 ~= 3.1% (integer-uniform "
    "draws make bitlen-92 the same ~3-4% conditional slice as every "
    "prior cell; same rejection protocol as exps 517..536); corrected "
    "windows are also exactly the ladder's next rung (+4 exponents over "
    "exp536's [2^38,2^44)x[2^44,2^50)). FLAGGED for parent."
)
...
NOTES = {
    ...
    "classifier": "ONE GLOBAL two-level block-gcd structure (LEAF=24/MID=18,"
                  ...
                  "unchanged from exp536 (proven + cross-checked identical "
                  f"to the exp528..536-verbatim classifier); checked on "
    ...
    "context_prior_cells": {
        ...
        "uniform_84_exp535_pooled_paper188": {
            "rho_T": 0.558, "rho_T_ci": [0.536, 0.581],
            "note": "margin to floor +0.008; gradual erosion, no cliff"},
        "uniform_88_exp536_pooled": {
            "rho_T": ANCHOR_88_RHO, "rho_T_ci": ANCHOR_88_CI,
            "note": "BELOW the 0.55 floor for the first time; "
                    "DRIFT-INCONCLUSIVE; this cell decides terminal-vs-plateau"},
    },
}
...
def smooth_count_sq(v, fb, B):
    """Reference classifier (exp528..exp536-verbatim form). Decides:
...
class GlobalClassifier(object):
    ...
    docstring, carried over unchanged through exp536):
...
def run_designed_checks():
    ...
    # masked classifier against the exp528..536-verbatim classifier at
    # multiple bounds (top bound rescaled to the bitlen-92 base size).
    ...
    n_x = 0
    for B_x in (100, 997, 200000, 456436):
        fb_x = make_fb(B_x)
        for _ in range(500):
            v = rng_x.randrange(1, 1 << 94)
            n_x += 1
    ...
    DESIGNED_CHECKS["smoothness_unit_tests"] = {
        ...
        "bounds_tested": [100, 997, 200000, 456436]}
...
# ------------------------------------------------------- state / io
STATE = {
    "experiment": "exp538", "codename": "TDIAL-U92", "round": 69,
...
def ledger(msg, **kw):
    ent = {"t_utc": now(), "line": msg}
    ent.update(kw)
    STATE["ledger"].append(ent)
    print(f"[LEDGER] {msg} {kw if kw else ''}", flush=True)
...
def main():
    ...
    STATE["config"] = {
        ...
        "windows_as_written_in_brief": [[36, 42], [42, 48]],
        "dial_cutoff": DIAL_CUT, "count_cutoff": COUNT_CUT, "bootstrap": BOOT,
        "fb_ceiling": FB_CEIL, "line_h1_h2": LINE,
        "ladder_floor_context": LADDER_FLOOR,
        "anchor88_rho": ANCHOR_88_RHO, "anchor88_ci": ANCHOR_88_CI,
    ...
    ledger("stage=config", seeds=SEEDS, n=N_PER_SEED, k=K_VALUES, u=U_EXP,
           boot=BOOT, smoke=SMOKE, line=LINE, ladder_floor=LADDER_FLOOR)
    checkpoint()
    ...
    n_seeds_ge_line = sum(1 for r in rhoTs if r >= LINE)
    n_seeds_ge_floor = sum(1 for r in rhoTs if r >= LADDER_FLOOR)

    # pre-stated operationalizations (see module docstring)
    h1_pass = bool(pool_rho < LINE and pool_hi < LINE)
    h2_pass = bool(pool_rho >= LINE)

    if h1_pass:
        name = "U92-TERMINAL-DRIFT"
    elif not h2_pass:
        name = "U92-DRIFT-INCONCLUSIVE"
    elif pool_rho < LADDER_FLOOR:
        name = "U92-PLATEAU-AT-FLOOR"
    else:
        name = "U92-PLATEAU-ABOVE-FLOOR"

    anchor_lo, anchor_hi = ANCHOR_88_CI
    STATE["hypothesis_verdicts"] = {
        "H1-terminal": {
            "statement": HYPOTHESES["H1-terminal"],
            "operationalization": "pooled rho_T < 0.50 AND pooled bootstrap "
                                  "CI upper < 0.50 ('decisively below')",
            "per_seed_rho_T": rhoTs,
            "n_seeds_ge_050": n_seeds_ge_line,
            "n_seeds_ge_055": n_seeds_ge_floor,
            "pooled_rho_T": pool_rho, "pooled_ci": pool["spearman_T_ci"],
            "margin_to_line_point": round(pool_rho - LINE, 4),
            "ci_excludes_line_below": bool(pool_hi < LINE),
            "pass": h1_pass,
        },
        "H2-plateau": {
            "statement": HYPOTHESES["H2-plateau"],
            "operationalization": "pooled rho_T >= 0.50",
            "per_seed_advanages_note": "advantages reported below",
        ...
        "anchor_comparison": {
            "anchor88_pooled": ANCHOR_88_RHO, "anchor88_ci": ANCHOR_88_CI,
            "delta_pooled_vs_anchor88": round(pool_rho - ANCHOR_88_RHO, 4),
    ...
    ledger("stage=verdict", verdict=name, H1=h1_pass, H2=h2_pass,
           ...
           n_seeds_ge_050=n_seeds_ge_line, n_seeds_ge_055=n_seeds_ge_floor)
    checkpoint()
    ...
    STATE["barriers"] = {
        ...
        "barrier_8_scope_line":
            ...
            "diagnostic range -- uniform semiprimes of bitlen exactly 92 "
            "(p ~ U[2^40,2^46), q ~ U[2^46,2^52) after the pre-data window "
            "correction), u = 2.5, 240 CFRAC values/N, n = 1200 x 3 seeds; "
            "no extrapolation to cryptographic parameterizations; zero "
            "fitted parameters on this cell (dial form sum 2/p over QR "
            "primes <=400 and count<=100 fixed a priori per paper-164; the "
            "pre-data window correction is a feasibility fix, not a fit; "
            "the global masked classifier is carried over semantics-"
            "preserved from exp536 and was locked before data).",
    }
    STATE["decision"] = (
...
def write_ledger_md(name, h1v, h2v):
    lines = [
        "# LEDGER -- exp538 TDIAL-U92 (round-69)",
        "",
        f"- date_utc: {now()}",
        ...
        f"descent, carried over semantics-preserved from exp536 "
