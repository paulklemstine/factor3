...
# -*- coding: utf-8 -*-
"""
exp530_t_dial_unif_64.py -- TDIAL-U64 (round-60, experiment 530)

Push the zero-fit dial T(N) = sum_{odd QR primes p<=400} 2/p to BITLEN 64
on UNIFORM draws (prior uniform cells: 44 -> exp517, 52 -> exp518/528,
56 -> exp529; balanced cells through bitlen 60).
...
DESIGN (as assigned)
==========
Work dir    : /tmp/exp60_tu64/   (protocol: work ONLY here; never write to
                                 /home/raver1975/factor3)
Population  : 1200 uniform semiprimes per seed; seeds 20261140, 20261141,
              20261142; N rejected unless bit_length == 64 exactly.
...
518, 521, 528 and 529 (each round's brief carries windows two octaves too
low for its target bitlen).
Correction applied before data collection, following exp529's rule exactly:
...
midpoints 29 + 35 = 64). Everything else in the brief is implemented
verbatim. FLAGGED for parent adjudication.

PROTOCOL NOTE (flagged): this agent READ (never wrote) two artifacts of the
immediate predecessor cell -- the exp529 script (repo scripts dir) and its
/tmp result.json -- to guarantee verbatim protocol fidelity. No repo file
was modified or created.

Implementation deltas vs exp529, both forced by bitlen 64 and neither a fit:
 * sieve ceiling 6000 -> 20000: B_N = (2*sqrt(N))^(1/2.5) reaches ~9.7e3 at
   bitlen 64 (vs ~3.1e3 at 56); the factor base must cover B_N.
 * everything else byte-for-byte the exp529 pipeline (population draw order,
...
VERDICT NAMES (pre-stated):
  H1 pass & H2 pass -> U64-DIAL-CONFIRMED
  H1 only           -> U64-DIAL-HOLDS-COUNT-PARITY
  H2 only           -> U64-DIAL-SHIFT-COUNT-ROBUST
  neither           -> U64-DIAL-BREAK
...
SCRIPT_PATH = os.path.join(WD, "exp530_t_dial_unif_64.py")
...
CORRECTION_TEXT = (
    ...
    "defect documented pre-data in exps 517/518/521/528/529). Pre-data "
    "correction per exp529's rule: both exponent windows shifted by the SAME "
    ...
    "uniform character; product-mode lands on 2^64. FLAGGED for parent."
)
...
def smooth_count_sq(v, fb, B):
    ...
    for speed; identical semantics to the exp528/529 classifier.)"""
    x = v
    for pr, pr2 in fb:
        if pr2 > x:
            break
...
# ------------------------------------------------------- state / io
STATE = {
    "experiment": "exp530", "codename": "TDIAL-U64", "round": 60,
...
def main():
    ...
    t2 = time.time()
    STATE["pooled"] = summarize(Ts, Cs, Rs, bs_seed=20260822)
    ...
    if h1v and h2v:
        name = "U64-DIAL-CONFIRMED"
    elif h1v and not h2v:
        name = "U64-DIAL-HOLDS-COUNT-PARITY"
    elif (not h1v) and h2v:
        name = "U64-DIAL-SHIFT-COUNT-ROBUST"
    else:
        name = "U64-DIAL-BREAK"
    ...
    STATE["barriers"] = {
        ...
        "barrier_8_zero_fit_discipline":
            ...
            "bitlen-64-uniform is an out-of-sample intersection fill; no "
        ...
        "note": "wording reconstructed from lab semantics (barrier 5 = "
                ...
                "papers 83/183/184 usage and the exp529 wording; repo "
                "untouched (read-only).",
    }
    STATE["decision"] = (
...
def write_ledger_md(name, h1v, h2v):
    lines = [
        "# LEDGER -- exp530 TDIAL-U64 (round-60)",
        "",
        f"- date_utc: {now()}",
