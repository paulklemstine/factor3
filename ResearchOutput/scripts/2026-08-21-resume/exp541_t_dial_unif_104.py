#!/usr/bin/env python3
"""EXP 541 'TDIAL-U104' (round-68) -- zero-fit dial T(N) pushed to bitlen 104, u=2.5.
...
to result.json at stage 00 before any population sampling):
  H1: Spearman(T, rate) CONTINUES FADING at bitlen 104 -- pooled
      Spearman(T, relation_rate) < 0.53.
  H2: The fade STABILIZES -- pooled Spearman(T, relation_rate) >= 0.53.
      Carry-in priors: pooled Spearman 0.5739 (fixed classifier) at bitlen 96;
      0.5436, CI [0.4982, 0.5881] at bitlen 100 (TDIAL-U100-FADES-AT-100 --
      first cell below the validated band [0.55, 0.85]).

DESIGN (fixed before data):
  Population : 1200 uniform semiprimes = 3 seeds x 400
               (seeds 20261210, 20261211, 20261212).
               p ~ uniform prime in [2^46, 2^52),
               q ~ uniform prime in [2^52, 2^58),
               rejection-sampled until bitlen(N) == 104 exactly.
               DESIGN NOTE / BRIEF CORRECTION: the tasking brief wrote
               "p in [2^16,2^22), q in [2^22,2^28)" -- those ranges cap
               N < 2^50 and are mathematically INCOMPATIBLE with the brief's
               own binding constraint bitlen(N) = 104 (max product < 2^50).
               The ranges used here are the established TDIAL ladder
               continuation (88/92/96/100 used width-6 octave windows meeting
               at 2^(B/2): e.g. bitlen 100 -> [2^44,2^50) x [2^50,2^56)),
               i.e. [2^(52-6),2^52) x [2^52,2^(52+6)) for B=104. Everything
               else is verbatim exp540/TDIAL-U100 methodology.
...
BARRIERS (standard lines):
  (5) SCOPE: claims restricted to uniform semiprime draws at bitlen exactly
      104, u = 2.5, and this specific relation-value construction; no claim
...
LO_P, HI_P = 2**46, 2**52
LO_Q, HI_Q = 2**52, 2**58
BITS_N = 104                     # exact bitlen requirement
...
H_THRESHOLD = 0.53               # pre-stated H1/H2 boundary

T_START = time.time()


def write_result(status, payload):
    doc = {"exp": "541", "codename": "TDIAL-U104", "round": 68,
...
def gen_population(seed):
    ...
    NOTE measured acceptance ~2-4%: uniform-over-primes tilts both factors to
...
def smooth_rates(N_list_ints, tag):
    ...
    for i, Nmpz in enumerate(N_list):
        ...
        for dd in d.tolist():
            x = r + dd
            vals.append(x * x - N)     # python ints; V < 2^62 here
        vmax = max(vals)
...
def main():
    ...
    write_result("00_hypotheses_stated", {
        "hypotheses": {
            "H1": "Spearman(T, rate) CONTINUES FADING at bitlen 104: "
                  "pooled Spearman < 0.53",
            "H2": "The fade STABILIZES: pooled Spearman >= 0.53",
        },
        ...
        "brief_correction": "brief's literal ranges [2^16,2^22)x[2^22,2^28) "
                            "cap N<2^50, incompatible with bitlen(N)=104; "
                            "corrected pre-data to the TDIAL-ladder windows "
                            "[2^46,2^52)x[2^52,2^58) (width-6 octaves meeting "
                            "at 2^(B/2), as in exp536..540)",
    ...
    h1 = bool(pT < H_THRESHOLD)          # fade continues
    h2 = bool(pT >= H_THRESHOLD)         # fade stabilizes
    ci_excludes = ("below", "straddles", "above")[
        0 if ciT[1] < H_THRESHOLD else 1 if ciT[0] <= H_THRESHOLD else 2]
    further_decline = bool(pT < PRIOR_100)
    h2_seed = bool(all(ps["rho_T"] >= H_THRESHOLD for ps in res["per_seed"]))
    if not h1:
        verdict = "TDIAL-U104-STABILIZES"
    else:
    ...
    write_result("04_final", {
        ...
        "comparisons": {"prior_rho_96_fixed": PRIOR_96,
                         ...
                         "delta_vs_100": pT - PRIOR_100,
                         "ci95_vs_threshold": ci_excludes,
                         "further_decline_vs_100": further_decline},
        "verdicts": {"H1_pass": h1, "H2_pass": h2,
                     "H2_pass_per_seed": h2_seed,
