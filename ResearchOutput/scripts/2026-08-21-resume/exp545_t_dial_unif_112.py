#!/usr/bin/env python3
"""EXP 545 'TDIAL-U112' (round-70) -- zero-fit dial T(N) pushed to bitlen 112,
u=2.5.
...
tasking brief):
  ...
  Carry-in priors (TDIAL-U ladder): pooled Spearman 0.5739 (fixed
      ...
      (TDIAL-U108-CONTINUES-FADE). NOTE: the brief quotes the validated-band
      H1 against a carry-in chain whose last THREE cells already sit BELOW
      the band's lower edge 0.55 -- the hypotheses are tested exactly as
      stated.
...
DESIGN (fixed before data):
  Population : 1200 uniform semiprimes = 3 seeds x 400
               ...
               "p uniform prime in [2^24,2^30), q uniform prime in
               [2^30,2^36)" -- those ranges cap N < 2^66 and are
               mathematically INCOMPATIBLE with the brief's own binding
               constraint bitlen(N) = 112 (rejection sampling would accept
               nothing). This is the SAME brief-typo pattern as exp541
               (bitlen 104) and exp544 (bitlen 108), shifted +4; both prior
               cells corrected it pre-data to the established TDIAL ladder
               continuation (width-6 octave windows meeting at 2^(B/2):
               bitlen 104 -> [2^46,2^52) x [2^52,2^58); bitlen 108 ->
               [2^48,2^54) x [2^54,2^60)), i.e. HERE
               [2^50,2^56) x [2^56,2^62) for B=112. Everything else is
               verbatim exp544/TDIAL-U108 methodology.
  ...
  Smoothness : per-N bound B_N = ceil(Vmax^(1/2.5)) where Vmax is the max of
               ...
               factoring anywhere.
               CLASSIFIER EXTENSION (required at B=112, documented pre-data):
               relation values now reach 2rd + d^2 ~ 2^65.001 > 2^64-1, so
               exp544's single-uint64 storage would OVERFLOW. Values are held
               as an exact 34/34 split-word pair (V = VH*2^34 + VL, both
               uint64); every remainder/division step is the standard exact
               long-division identity on the split (intermediate products
               bounded < 2^61), so the classified predicate -- smooth wrt the
               row's OWN bound exactly -- is bit-identical to exp544's.
  ...
  Runtime    : per-seed rate computation parallelized as 3 seeds x 3 row-
               ...
               exp544's sequential-per-seed loop).

BARRIERS (standard lines):
  (5) SCOPE: claims restricted to uniform semiprime draws at bitlen exactly
      112, u = 2.5, and this specific relation-value construction ((r+d)^2-N,
  ...
  (8) MEASUREMENT: rate is a 240-sample smoothness proxy (binomial sigma
      ...
      (definition-dependence).  Classifier is the exact split-word extension
      of the spot-check-asserted exp540-fixed one (overflow-driven, documented
      above); the brief's factor-range typo was corrected to the
...
# ---------------- fixed configuration ----------------
SEEDS = [20261210, 20261211, 20261212]
N_PER_SEED = 40 if SMOKE else 400     # 3 x 400 = 1200 total
LO_P, HI_P = 2**50, 2**56
LO_Q, HI_Q = 2**56, 2**62
BITS_N = 112                     # exact bitlen requirement
...
def write_result(status, payload):
    doc = {"exp": "545", "codename": "TDIAL-U112", "round": 70,
           "smoke": SMOKE,
...
def gen_population(seed):
    ...
    NOTE measured acceptance ~3% at bits 100/108 (uniform-over-primes tilts
    both factors to the top octaves, so exact-bitlen products are rare) --
...
def _pollard_brent(n):
    ...
    NOTE: n reaches ~3.7e19 at bitlen 112 (> int64 max), so the draws use
...
def build_relation_values(N_list_ints, tag):
    ...
    for Nmpz in N_list_ints:
        ...
        for dd in d.tolist():
            x = r + dd
            row.append(x * x - N)          # python ints; V reaches ~2^65.001
        vmax = max(row)
...
def classify_chunk(VHi, VLo, BVb, tag):
    ...
    BV. Semantics identical to exp544's exp540-fixed classifier: each row is
...
def main():
    ...
    write_result("00_hypotheses_stated", {
        ...
        "brief_correction": "brief's literal ranges [2^24,2^30)x[2^30,2^36) "
                            "cap N<2^66, incompatible with bitlen(N)=112; "
                            "corrected pre-data to the TDIAL-ladder windows "
                            "[2^50,2^56)x[2^56,2^62) (width-6 octaves meeting "
                            "at 2^(B/2), as in exp536..544; same brief-typo "
                            "pattern as exp541/exp544, shifted +4)",
        "classifier_extension": "relation values reach 2rd+d^2 ~ 2^65.001 > "
                            "uint64max at bitlen 112; values held as exact "
                            "34/34 split words (V=VH*2^34+VL), remainder/"
                            "division exact integer identities, predicate "
                            "identical to exp544's uint64 classifier; "
    ...
    write_result("04_final", {
        ...
        "comparisons": {"prior_rho_96_fixed": PRIOR_96,
                         ...
                         "further_decline_vs_108": further_decline},
