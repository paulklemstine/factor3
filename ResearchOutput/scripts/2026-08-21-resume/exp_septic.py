#!/usr/bin/env python3
"""SEPTIC-FRONTIER — extending beyond degree 6 (round-34 #2)."""
...
print("=== SEPTIC-FRONTIER (round-34 #2): degree 7 ===",flush=True)
pf=sieve(1<<16)
prp=pf[~np.isin(pf,[3,7])]
NM=30000

# x7-3: root counts mod p
coeffs=(-3,0,0,0,0,0,1)
...
# type = root count (simplification; full pattern needs more machinery)
types=nr_all.copy()
...
print(f"  distinct types={len(tc)}",flush=True)

# measure MI at various moduli
print(f"\nMI at moduli:",flush=True)
for m in (3,5,7,13,19):
    pm=(prp%m).astype(np.int64)
    v=mi(pm,types)
    nl=[]
    rng_np=np.random.default_rng(m*100+7)
    for _ in range(100):nl.append(mi(rng_np.permutation(pm),types))
...
pc=(np.minimum(tP,tQ)*10+np.maximum(tP,tQ)).astype(np.int64)
Nm7=(PS*QS)%7
I_pair=mi(Nm7,pc)
rng_np2=np.random.default_rng(777);nl2=[]
for _ in range(200):nl2.append(mi(rng_np2.permutation(bg),pc))
z_pair=(I_pair-np.mean(nl2))/(np.std(nl2)+1e-12)
wf=mi(bg,pc)
print(f"\nSEMIPRIME LEVEL:",flush=True)
print(f"  I(N mod 7; pair)={I_pair:.4f} | wall z={z_pair:+.2f}",flush=True)
...
print("Round-34 #2.",flush=True)
print("\nALL_DONE_R34N2",flush=True)
