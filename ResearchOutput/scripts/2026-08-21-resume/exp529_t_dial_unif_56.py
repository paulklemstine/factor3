...
# -*- coding: utf-8 -*-
"""
exp529_t_dial_unif_56.py -- TDIAL-U56 (round-59, experiment 529)

Fill the next open intersection for the zero-fit dial
T(N) = sum_{odd QR primes p<=400} 2/p:
UNIFORM draws at BITLEN 56 (prior cells: balanced draws through bitlen 60,
uniform draws through bitlen 52).
...
DESIGN
==========
Work dir    : /tmp/exp59_tu56/   (protocol: work ONLY here; never write to
                                 /home/raver1975/factor3)
Population  : 1200 uniform semiprimes per seed; seeds 20261130, 20261131,
              20261132; N rejected unless bit_length == 56 exactly.
...
the same +8 (the identical pre-data correction experiment 528 applied to the
bitlen-52 brief):
...
and places the product distribution's MODE exactly at 2^56 (window-log
midpoints 25 + 31 = 56), so the bitlen-exact-56 conditioning distorts the
population least (~13% acceptance). Everything else in the brief is
implemented verbatim. FLAGGED for parent adjudication.

PROTOCOL NOTE (flagged): this agent READ (never wrote) two repo files to
guarantee verbatim protocol fidelity -- the exp528 script (prior uniform
cell, one bitlen down) and its round record 183_TDialUnif52.md. No repo
file was modified.
...
SCRIPT_PATH = os.path.join(WD, "exp529_t_dial_unif_56.py")
...
CORRECTION_TEXT = (
    ...
    "lands on 2^56 (minimal conditioning distortion). Same +8 correction "
    "pattern exp528 applied pre-data at bitlen 52. FLAGGED for parent."
)
...
def smooth_count_sq(v, fb, B):
    ...
    for speed; identical semantics to the exp528 classifier.)"""
    x = v
    for pr, pr2 in fb:
        if pr2 > x:
            break
...
# ------------------------------------------------------- state / io
STATE = {
    "experiment": "exp529", "codename": "TDIAL-U56", "round": 59,
...
def main():
    ...
    if h1v and h2v:
        name = "U56-DIAL-CONFIRMED"
    elif h1v and not h2v:
        name = "U56-DIAL-HOLDS-COUNT-PARITY"
    elif (not h1v) and h2v:
        name = "U56-DIAL-SHIFT-COUNT-ROBUST"
    else:
        name = "U56-DIAL-BREAK"
    ...
    # canonical barrier table was NOT modified -- read-only protocol note above)
    STATE["barriers"] = {
        ...
        "barrier_8_zero_fit_discipline":
            ...
            "bitlen-56-uniform is an out-of-sample intersection fill; no "
...
def write_ledger_md(name, h1v, h2v):
    lines = [
        "# LEDGER -- exp529 TDIAL-U56 (round-59)",
        "",
        f"- date_utc: {now()}",
