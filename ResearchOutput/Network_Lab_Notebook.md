...
## Part 105R (quarantine note): MOE-HOT-SET enforcement UNAVAILABLE on this stack — user-run scopes lack memory.max entirely (10s polling, three designs: hand-cgroup bounced by systemd, --user scope without memory files); system-scope MemoryMax does not bind either (NET-105 sudo run: RSS 16.26GB under 6G cap). Environment limitation documented; MoE-routing-under-pressure question REMAINS OPEN pending proper delegation fix. Round quarantined, no count increment. PROGRAM PIVOT per goal: GPTOSS-120B-STREAMTAX requires NO cgroups — at 60.8GB weights vs 31GB RAM the model lives in NET-104's starved-streaming regime BY PHYSICAL NECESSITY; download started.

## Part 106 — GPTOSS-120B-STREAMTAX BLOCKED-BY-LOAD (quarantine note): the 116.8B capstone CANNOT LOAD on this box in the current llama.cpp build — llama_model_load fails with "unable to allocate CPU_REPACK buffer of size 60914073600" (60.9GB): the MXFP4 loader demands a full-size contiguous repack buffer that exceeds 31GB RAM. This is a LOAD failure, not a streaming-feasibility measurement — the round is quarantined without count increment. Mitigations queued (not run): (a) ik_llama.cpp fork may handle MXFP4 repack differently (its loader/repack path differs); (b) check for a --no-mmap / buffer-type-override flag; (c) a lower-total-param native-MXFP4 target (gpt-oss-20b already measured at 9.92 tok/s, NET-102) remains the validated endpoint. The 63.4GB download is retained on disk pending a mitigation build. THE GOAL is otherwise demonstrated at 14B-in-6.5GB (NET-99) and 30B-A3B-in-13.3-13.9 tok/s (NET-103); the 120B load blocker is a toolchain limitation, not a physics one.
