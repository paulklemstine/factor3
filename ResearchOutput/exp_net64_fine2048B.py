...
#
# PREDICTIONS STATED BEFORE THE RUN (corpus-B @2048, resolving its coarse-grid 32):
#  P1 SHARD-DIFFERENCE-REAL: corpus-B's fine knee is genuinely higher (k*=28 or 32) than
#     corpus-A's 24 — shard-level attention structure differs at long context.
#  P2 WINDOW/COUNT-ARTIFACT: corpus-B's fine knee lands at 24 too — the coarse 32 was a
#     grid/window artifact, and the knees are corpus-robust here as at 512/1024.
#  P3 BETWEEN: fine knee = 28 (between A's 24 and B's coarse 32).
...
CORPUS = os.path.expanduser("~/f3cache/net49_corpus_b.txt")   # corpus A
...
def main():
    ...
    res = {"ctx": CTX, "corpus": "B", "nw": len(win), "full_acc": round(full_acc, 5),
           "cells": []}
