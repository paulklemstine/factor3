...
## Part 212 - QR-REPLICATION (round-39 #2 [cron iteration], exp 475, v249)

Fresh-seed audit of paper 139 (seed 20260826 vs original 20260821) — the lab's
reproducibility-audit discipline applied to its newest result, one firing after a seed-luck
embarrassment (paper 136). EVERY HEADLINE REPLICATES EXACTLY:
Ensemble: emp_x2 ~= emp_rnd at all four cells (0.12859/0.12786, 0.02004/0.02023,
0.12854/0.12585, 0.02004/0.01869); QR-restricted randoms 32-200x lower.
Per-N: corr(QR-count<=100, per-N rate) = 0.503/0.415/0.480/0.403 vs original
...
CELL-CLOSED-DIAL-HOLDS — Spearman(T, rate) = 0.793/0.808/0.808 across 3 seeds;
H1 PASS; H2 CONFIRMED (pooled advantage +0.121 CI [0.103,0.140]); the dial survives the
intersection of regime-invariance and bitlen-stability at its highest tested point.
Paper 182, issue next. Now 517 experiments.

## Part 248 - T-DIAL-UNIF-48 (round-52 #1 [cron iteration], exp 517, v285)

The zero-fit dial on uniform draws at bitlen 48: H1 CONFIRMED (Spearman(T) = 0.777/0.755/
0.801 across 3 seeds, all in [0.55, 0.85]); H2 CONFIRMED (T beats count by +0.11/+0.14/
+0.13 on all seeds). The dial's regime-invariance extends to bitlen 48.
Paper 182, issue next. Now 517 experiments.
...
## Part 277 - WEIGHT-EXPONENT-FIT (round-83 #2, exp 586, v342)

ERRATUM-GRADE SUPERSESSION of a recorded paper's adopted parameter. Paper 227 (exp577) adopted
the 1/ℓ-harmonic weight for the canonical scale-smoothness product dial BY INSPECTION; exp586
fit the optimal exponent α properly on the SAME data (pure reanalysis of exp577's per-N hit
counts, no new j-sampling). Verdict: **H1_HARMONIC_REFINED — the dial weight exponent is
α̂ = 0.5 (√-weight), not 1**.

Pre-registration was written into the script docstring BEFORE analysis (H1 iff α̂≠1 AND
ΔR² ≥ 0.03; H0 iff CI contains 1 OR ΔR² < 0.02; else BORDERLINE). Method: regenerate the same
128 Ns verbatim from exp577's recipe (seed 20260827, bitlen 96), odd primes 3..400 only (paper
231 lesson), mechanistic Legendre c_ℓ = [jacobi(N mod ℓ, ℓ) = +1], S_α(N) = Σ c_ℓ/ℓ^α, regress
log((hits+0.5)/total) ~ S_α by OLS over grid {0.25,...,2.0}, bootstrap argmax over 500 Ns-resamples.

Alpha curve — SINGLE-PEAKED INTERIOR MAX at 0.5, harmonic sits on the FALLING limb:
α:        0.0    0.25   **0.5**   0.75   1.0    1.25   1.5    2.0
R²:       .3207  .4985  **.6242** .5752  .4731  .3969  .3479  .2944

ΔR²(α̂) − R²(1) = +0.15114 ≥ 0.03 bar → H1 fires. Bootstrap CI95 = [0.5, 0.5] (492/500 reps pick
0.5, 8/500 pick 0.75, mean 0.504) — EXCLUDES 1 DECISIVELY. Sanity anchors vs paper 227's sweep:
even harmonic beats unweighted +0.152385 (227 was right that weighting matters); best beats
unweighted +0.303524 (wrong about the exponent).

SUPERSESSION: paper 227's adopted covariate 1/ℓ → refined 1/√ℓ lab-wide. Large primes carry
~√ℓ× more relative weight than the harmonic form assumed; the fix lifts dial explanatory power
from R²=.473 to R²=.624 (+31% relative) on identical data. The inspection-chosen 1/ℓ
over-penalized the tail; the true profile decays ℓ^-0.5. DOWNSTREAM: every future scale-smoothness
dial computation uses √-weight. Named follow-up honestly: window-location saturation (B*=400)
was measured UNDER 1/ℓ (papers 136/139/227); its transfer to the refined √-weight is UNVERIFIED
and must be re-checked before B* claims are reused.

Integrity: 128/128 Ns regenerated byte-identical from recipe; recomputed odd-prime QR counts
match exp577's stored S400 column EXACTLY (diff 0.0, corr ≈1.0) — also confirming exp577's dial
effectively excluded ℓ=2 despite the config ambiguity flagged in honest_notes. Limits disclosed:
discrete grid ±0.25 resolution (fine structure near 0.5 not fit per pre-registration); single
seed 20260827, pure reanalysis no fresh replicate; OLS log-rate Poisson attenuation applies
UNIFORMLY across α so argmax is likely robust while absolute R²s are attenuated; smoke n=16
picked grid-edge 0.25 (small-n noise, full run interior). Ledger catch: task-brief schema drift
only (raw_counts keys vs rows layout) — no data issue.

Count 572 → 573; max-id tracker 572 → 586 (no exp584/585 artifacts exist in the repo — 586 is
the new highest id on record). Assessment v341 → v342.
...
## Part 279 - NEIGHBOR-SMOOTHNESS-COVARIATE (round-83 #4, exp 585, v344)

CLOSES paper 234's ranked queue item #1 as a FOUR-WAY NEGATIVE, converting the ~40% u≈10 rate
residual from "not yet explained" to **"tested-and-failed across four independent N-property
classes"**. Question: does LOCAL FACTORIZATION STRUCTURE AROUND N — omega(N±δ), log-LPF(N±δ),
δ ∈ {−8..8}\{0} (2048 gmpy2 factorizations) — predict per-N hit-richness of v_j = j²−N beyond
the QR dial? Verdict: **H0_SUPPORTED (nothing-beyond-QR)**.

Pre-registration in script header BEFORE analysis; one disclosed SMOKE-STAGE ENDPOINT AMENDMENT
before any lineage analysis: hits_full measured ZERO events in 160k+ synthetic draws even at
t≤16 → PRIMARY swapped to SIQS-partial hits (400-smooth part × one prime ≤2^40); secondary
recorded. Gates unchanged.

Hierarchical regression (PRIMARY; n=128 bitlen-96 semiprimes, 50k j-samples/N, t~U[1,64],
μ=4032 hits/N, CV 1.57%):

| Model | R² |
|---|---|
| QR alone (Σ 2/p over odd p≤400, (N/p)=1) | .4112 |
| NB alone [ω̄−, ω̄+, lpfloḡ−, lpfloḡ+] | .0319 |
| Joint | .4307 |
| **ΔR²(neighbors)** | **+0.01946** |

Gate H1 ≥0.05 & perm p<0.01 / H0 <0.02: observed lands AT the boundary's underside AND the
permutation forecloses any effect claim — p=0.389 (500 joint row-shuffles), null q95=0.0457 >
observed (four noise predictors inflate more than this by chance). Best single neighbor |r|=0.16
vs dial r=0.641 (QR replication fires exactly where papers 227/235 say — pipeline live).
Asymmetry: ΔR²(QR given NB)=+0.3987, no shared carrier. Secondary hits_full: ΔR²=+0.0126, same
story. Refinement note: the QR block used the harmonic-family form (pre-235 canonical); the
adopted √-weight B*=400 dial absorbs MORE, so room for any neighbor increment is SMALLER than
measured — papers 235/236 strengthen this H0.

MAP UPDATE (four-class table): (1) QR/scale-smoothness dial REAL (227/235/236, R² .41-.62, the
one live carrier); (2) arithmetic-neighborhood structure NULL (this); (3) positional geometry an
INDEPENDENT layer carrying its own structure, not the rate residual (228-230); (4) S_indiv
consistent null (226/227). Whatever carries the residual is NOT any tested N-property form;
consistent with round-81's sole-survivor channel (window/polynomial geometry of j²−N itself,
issue #379) — candidate carriers belong to sieve-process/j-sample-level structure, not richer
functions of N.

LEDGER CATCHES: (1) PROVENANCE UNVERIFIED — exp581_regen_positions.npz holds only hit/control
POSITION arrays + jlo/jhi, NO N array; population REGENERATED by documented recipe seed 20260827;
isqrt fingerprint vs lineage positions 0/128 match, 15-variant recipe grid all 0-match
(exp577_result.json unreadable); computed hash 30009f0cfa7f85118d217bcacadb9d656fe5a1ae33b474b42
1492e828066bfef recorded. Results CONDITIONAL on balanced-96-bit exchangeability across seeds
(lab precedent: these laws population-robust), NOT the identical-N lineage. (2) Smoke-stage
endpoint amendment (above; itself a finding — fully-B-smooth relations are not a measurable
per-N endpoint at bitlen 96, QS relations there are ALL partials). (3) Wall 47.3 s (covariates
42.7 s dominant). Limits: hit classification reimplemented in-session (exp569 source unreadable),
t≤64 window ours not canonical exp578 range; 20/2048 neighbor factorizations ρ-censored and
median-imputed, rows_with_imputation=0.

Count 574 → 575; max-id tracker unchanged (587 stays highest on record). Assessment v343 → v344.
