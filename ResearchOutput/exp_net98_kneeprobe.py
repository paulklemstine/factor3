#!/usr/bin/env python3
# NET-98 — SUB-8 KNEE PROBE (continuation of NET-97's open lower bracket):
...
#  P1 KNEE-AT-SIX: k=4 FAILS (<0.98) while k=6 PASSES => k*(7B@512)=6.
#  P2 DEEPER-BOTTOM: k=4 also PASSES => knee <=4 (shrink strengthens).
#  P3 MONOTONE across {4,6,8(reused from NET-97)}.
...
CTX, NW = 512, 6
KGRID = [4, 6]
RESULTS = os.path.expanduser("~/f3cache/net98_results.json")


def log(msg):
    print(msg, flush=True)
    with open("/tmp/net98.log", "a") as f:
        f.write(str(msg) + "\n")
