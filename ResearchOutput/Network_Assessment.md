...
- **NET-89 — THE-MIXED-DOMAIN-STARTS-LOW-AND-RISES-FAST (limited-memory axis round 33; first mixed-domain cell): interleaved code+prose gives knees {12, 20} at {512, 1024} — starting at CODE's level but rising at DOUBLE the expected rate (+8/doubling vs +4 for either pure domain) to reach PROSE's level by 1024; P1 average REFUTED, P2 partial, P3 confirmed in spirit; the mixed domain has its OWN attention structure shaped by cross-domain query-key interactions; opens new sub-axis: domain-mixing effects on attention budgets; baseline acc between components. Gate exact; 50/50 ratio, one block size stated. Barriers: (a) clean; (b) clean; (c) confronted; (d) clean; (e) deterministic; (f) clean; (g) fair; (h) DIRECT. Open: mixing-ratio sweep; block-size sensitivity; other pairs; 1.5B mixed; 7B cell. Paper 170, issue #333.

Assessment v90. 90 experiments (NET-1 through NET-90).

- **NET-90 — THE-SYMMETRIC-MIXTURE-BUMP (limited-memory axis round 65; the mixing-ratio sweep): ALL THREE pre-registered shapes REFUTED — knee is not linear in ratio (P1✗), not a dip (P2✗ — a BUMP above both pure domains), not monotone in prose fraction (P3✗); 50/50 code+prose needs {16, 20} keys at {512, 1024} while both pure endpoints and both asymmetric mixtures sit at {12, 16} — symmetric mixing pays +4 keys (+25–33%) at both contexts; double tax (lowest full acc 0.441/0.467 AND highest knee); code endpoint replicates NET-68 exactly, prose ±1 grid step (knee fuzz); mixed knees carry construction-draw variance larger than pure replication. Gate exact; 12 windows stated. Barriers: (a)–(h) clean/DIRECT as listed in paper. Open: other pairs; block size; 1.5B mixed; @4096 interaction; 7B cell. Paper 171, issue #334.

Assessment v91. 91 experiments (NET-1 through NET-91 pending; NET-91 CPU-speculative-decoding axis now open).

- **NET-91 — THE-CHEAP-DRAFT-WINS-AND-CODE-DRAFTS-DEEP (cpu-large-model axis round 66; first speculative-decoding cell): 7B Q4_K_M fully on CPU via llama.cpp with same-family drafts — up to 1.661× net throughput (0.5B draft d=8 code), 6/12 configs >1.05×; P1 refuted by a hair (47.72% vs ≥50%), P2 CONFIRMED dramatically, P3 REFUTED (no crossover — cheaper 0.5B wins all six head-to-heads despite lower acceptance: draft-cost dominance is the CPU-specific law), P4 CONFIRMED (+17.8 pts code>prose acceptance, growing with depth); optimal draft depth is DOMAIN-PARAMETERIZED (prose collapses to net loss at d=8, code pays through d=8); doubles as hardware-stability canary: ~55 min sustained full-CPU post-fix with zero incidents. Gate: overhead-calibrated wall-clock medians, greedy deterministic, 12 configs × 8 runs stated. Barriers (a)-(h) clean/DIRECT (paper). Open: per-position acceptance maps; adaptive depth; KV-quant ladder; weight-quant floors; knee transfer @7B. Paper 172, issue #345.

Assessment v92. 92 experiments (NET-1 through NET-92 pending).

- **NET-92 — THE-KV-CLIFF (cpu-large-model axis round 67; first KV-cache quantization cell): all q8_0 cache arms quality-free at ctx 2048 (worst +0.097% vs f16 control) while joint q4_0 KV ANNIHILATES the model (PPL 7.11 → 2,714.6, +38,084%) — the axis has no usable middle; P1 CONFIRMED (<1%), P2 CONFIRMED four orders past prediction (>5% stated), P3 unresolved by design (floor hides asymmetry; no 4-bit single-sided arms); measured +16–26% pass-time tax for quantized KV = memory-vs-speed trade, not memory-vs-quality. Gate: llama-perplexity on 62K-token held-out slice, threads=8, one model stated; SEs not captured (documented). Open: block-scaled 4-bit variants; single-sided 4-bit; cliff-vs-context; spec-decode interaction. Paper 173, issue #351.

Assessment v93. 93 experiments (NET-1 through NET-93 pending).
...
- **NET-98 — THE-KNEE-IS-EIGHT-EXACTLY (cpu-large-model axis round 73; compact): both horns refuted — k=4 fails (0.926) AND k=6 fails (0.962), so k*(7B@512)=8 exactly; NET-97's "<=8" tightens to "=8": the scale-shrink is an exact halving {16->8}; seven-point curve strictly monotone; gate bit-exact; baseline reproduced exactly third consecutive time. Addendum to paper 176 lineage. Issue #409.

Assessment v99. 99 experiments (NET-1 through NET-99).
